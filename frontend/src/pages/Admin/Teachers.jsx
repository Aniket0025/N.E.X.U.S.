...
const Teachers = () => {
  const [newTeacher, setNewTeacher] = useState({ name: '', email: '', subject: '' ,password:'',grade:''});
  ...
  const handleAddTeacher = async (e) => {
    ...
    if (newTeacher.name.trim() !== '' && newTeacher.email.trim() !== '' && newTeacher.subject.trim() !== '' && newTeacher.password.trim() !== '') {
      try {
        ...
        setNewTeacher({ name: '', email: '', subject: '',password:'',grade:'' });
      } catch (error) {
        ...
        <TeachersContent>
          ...
          <TeacherList>
            {teachers.map((teacher) => (
              <TeacherItem key={teacher.id}>{teacher.name} - {teacher.email} - {teacher.subject}-{teacher.password}-{teacher.grade}</TeacherItem>
            ))}
          </TeacherList>
        </TeachersContent>
      </Content>
